...
The private key stays in ~/.agentcage/signing-key.json (0600) and never leaves
...
Redirect it; a terminal is refused so the key never lands in scrollback.`,
		Example: `  agentcage keys export > agentcage-signing.key
  agentcage keys export | ssh ci 'agentcage keys import'`,
		Args: cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			if f, ok := cmd.OutOrStdout().(*os.File); ok && term.IsTerminal(int(f.Fd())) {
				return fmt.Errorf("refusing to print a private key to a terminal; redirect it: agentcage keys export > agentcage-signing.key")
			}
...
a different key needs --force.`,
		Example: `  agentcage keys import < agentcage-signing.key`,
		Args:    cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
...
the publisher's new key out of band.`,
	...
	rm := &cobra.Command{
		...
		Example: `  agentcage trust ls
  agentcage trust rm ghcr.io/okedeji`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !trust.Remove(args[0]) {
				return fmt.Errorf("no pinned key for %s; 'agentcage trust ls' shows what is pinned", args[0])
			}
