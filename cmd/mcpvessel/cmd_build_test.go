...
func TestRunBuild_HappyPath(t *testing.T) {
	srcDir := t.TempDir()
	if err := os.WriteFile(filepath.Join(srcDir, "Vesselfile"), []byte(
		"FROM python:3.12-slim\nENTRYPOINT python3 agent.py\n",
	), 0o644); err != nil {
		t.Fatalf("WriteFile Vesselfile: %v", err)
	}
...
func TestValidateEvalSuite(t *testing.T) {
	writeVesselfile := func(dir, eval string) {
		body := "FROM x\nMAIN respond\nENTRYPOINT y\n"
		if eval != "" {
			body = "FROM x\nMAIN respond\nEVAL " + eval + "\nENTRYPOINT y\n"
		}
		if err := os.WriteFile(filepath.Join(dir, "Vesselfile"), []byte(body), 0o644); err != nil {
			t.Fatalf("write Vesselfile: %v", err)
		}
	}

	t.Run("valid suite passes", func(t *testing.T) {
		src := t.TempDir()
		writeVesselfile(src, "tests/eval.yaml")
	...
	t.Run("escaping path fails", func(t *testing.T) {
		src := t.TempDir()
		writeVesselfile(src, "../outside.yaml")
		err := validateEvalSuite(buildConfig{srcDir: src})
...
func TestRunBuild_PropagatesBundleError(t *testing.T) {
	// Source dir has no Vesselfile, so bundle.Build returns an error.
	srcDir := t.TempDir()
