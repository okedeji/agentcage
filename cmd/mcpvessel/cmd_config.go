...
func newConfigCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: `Manage ~/.mcpvessel/config.json: the LLM endpoints the LLM gateway routes to, the
resource caps the runtime enforces, and where the daemon serves Prometheus metrics.

Provider keys are stored by reference: set the value with 'mcpvessel secrets set'
and point an endpoint at it with --key-ref. The config file never holds a secret.`,
		Example: `  mcpvessel config provider set openai --base-url https://api.openai.com/v1 --key-ref openai_key --default
  mcpvessel config resources set @okedeji/researcher:0.1 --memory 2g --cpus 2
  mcpvessel config cages set --max-live 64 --prewarm 16
  mcpvessel config metrics set 0.0.0.0:9323
  mcpvessel config show`,
	}
	cmd.AddCommand(
...
func newConfigEnvCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "env",
		Short: "Persist an VESSEL_* setting instead of exporting it each shell",
		Long: `Persist an mcpvessel environment knob in config.json so it survives across
...
'mcpvessel secrets'. A real environment variable of the same name overrides the
stored value, so a shell or CI override still wins for that run.`,
		Example: `  mcpvessel config env set VESSEL_GITHUB_CLIENT_ID Iv1.abc123
  mcpvessel config env ls
  mcpvessel config env rm VESSEL_GITHUB_CLIENT_ID`,
	}
	set := &cobra.Command{
		Use:   "set NAME VALUE",
...
func newConfigMachineCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "machine",
		Short: "Set how much of the host mcpvessel may use for cages",
...
machine is next provisioned ('mcpvessel init --recreate'). A zero means the
built-in default.`,
	}
...
A change takes effect when the daemon next starts.`,
		Example: `  mcpvessel config metrics show
  mcpvessel config metrics set 0.0.0.0:9323
  mcpvessel config metrics off`,
	}
...
func newConfigProviderSetCmd() *cobra.Command {
	...
	cmd.Flags().StringVar(&keyRef, "key-ref", "", "name of a secret (mcpvessel secrets) holding the API key")
...
// micro-USD. More than six decimal places is finer than mcpvessel tracks
// and is rejected.
