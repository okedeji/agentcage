...
TARGET is a reference ('mcpvessel build -t' put it in the store), the content
...
'mcpvessel run' does, so LLM tokens cost real money.
...
bundle's manifest, so 'mcpvessel inspect' and anyone who pulls the agent can see
...
func runSuiteForBundle(ctx context.Context, p suiteParams) (*eval.Report, error) {
	...
	if _, err := client.Version(ctx); err != nil {
		return nil, fmt.Errorf("cannot reach the mcpvessel daemon, run 'mcpvessel init' to start it: %w", err)
	}
