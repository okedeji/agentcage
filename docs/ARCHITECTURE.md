...
**Per-cage networks (I1).** No two cages share a network. This is the substrate every other guarantee rests on: because a cage's only network peers are the brokers multi-homed onto its network, a caged server has no route to a sibling and therefore cannot reach one except through the MCP gateway, where mediation applies. The root cage and any always-warm cage (one the operator keeps booted between calls; §10) hold a dedicated network. Cages that boot on demand draw a network from a small reusable pool, with one cage occupying a pool network at a time; the exclusivity holds across reuse, so a recycled network never carries two live cages at once. Pooling exists only to keep the number of networks bounded on a large tree, below the practical limits of the container network layer; it never relaxes exclusivity. A boot that cannot establish a cage's network fails rather than colocating it.

**Two kinds of network, and one door out.** Every cage network is *internal*: it has no route off the machine, and its only members are one cage and the brokers serving it. A run that touches the outside at all also holds one *egress network*, the sole non-internal network in the run, and exactly two containers ever join it: the egress proxy and the LLM gateway. No cage does. A cage with egress reaches the internet only by speaking to the proxy over its own internal network; the proxy relays approved connections out through the egress network.

**No host path enters a cage.** A cage's filesystem is its image plus a writable layer discarded at teardown. Nothing is bind-mounted in: operator inputs for a run arrive as environment values gated by the manifest's declarations (§7), never as files, and no cage sees a host directory. On macOS the Lima VM mounts no host directory either, so caged code sits two filesystem boundaries away from the operator's files.
...
Allowing a hostname must not allow every address it might later resolve to. The proxy resolves an approved name itself, filters the answers to public addresses (loopback, private, link-local, unspecified, and multicast ranges are all refused), and dials the exact address it checked rather than the name, so a DNS answer cannot change between check and dial (I5). A name that resolves only to internal addresses is refused outright; without this, one approved hostname would be a rebinding pivot into a sibling cage, a broker, or the host.
...
The cage sends a request bearing a placeholder model name and a throwaway credential. The gateway substitutes the real provider key, rewrites the model field to the operator's configured choice, meters the token spend against the run's budget in integer micro-USD, and forwards the request to the provider. It reaches the provider directly over the egress network rather than through the egress proxy: the gateway is trusted and the provider is the operator's own configured endpoint, so it is not subject to the deny-default hold that governs a cage's egress. The flip side, that a reasoner's model traffic is thereby outside the allow-set, is recorded as a limitation in §14. It routes by the per-agent capability token embedded in the URL, not by the agent's name, so one agent cannot address another's model route. A loopback control surface, driven like the others by `nerdctl exec`, lets the daemon adjust the budget on a live run.
...
3. The gateway resolves the edge's capability token to the sub-agent cage, applies `DENY`, `BAN`, and the sub-agent's build-time catalog, and on success forwards the call to that cage's bridge at `:8000/mcp`.
...
**Verification by behavior.** A bundle's tool catalog is not taken from the author's word. The server is booted in a cage at build time and introspected, and the catalog is what it actually advertised; a declared `MAIN` tool is checked to exist there. The introspection boot is itself minimally privileged: an internal network, no LLM gateway, an egress proxy with an empty allow-set and nobody positioned to approve an addition, and no secrets beyond any the operator supplied for the boot. Only the server's startup executes; no tool is called. A `USES` dependency must name a concrete version and may not use a floating `latest`, so a dependency cannot drift out from under the parent's content hash.

The observed catalog is then authoritative everywhere a tool is addressed. The `serve` front door registers only tools the catalog marks public, and `call` refuses a tool the catalog does not, so an outside caller cannot address a tool the build never observed. Inside the tree, the MCP gateway pins each `USES` edge to its sub-agent's catalog: a `tools/call` to a name outside it is refused and `tools/list` is filtered to it, so what a parent may see is what the build observed, not what the running server later claims. The one soft spot is a bundle with no catalog at all, built without introspection and declaring no `MAIN` or `EXPOSE`; its edges are filtered by `DENY` and `BAN` alone (§14).

**Signing and trust.** Every bundle is signed on push and verified on pull. The signature is ed25519 over the bundle's OCI manifest digest together with the repository it was published to and the tag it was published as, so a valid signature cannot be replayed onto a different repository, and a registry cannot serve an older signed version under a newer tag: a tag pull whose signature names a different version is refused as a downgrade. A pre-tag-binding signature (artifact schema before 0.2) is likewise refused rather than accepted with the weaker guarantee. Trust is established on first use: the first pull of a publisher pins the key it sees and prints its fingerprint, and every later pull must match that pin, with a changed key failing closed and loudly rather than swapping silently, the model SSH uses for host keys. A signature proves origin, not intent; it attests who built a bundle, and the sandbox, not the signature, is what contains a bundle that proves malicious. The full policy is in [SECURITY.md](../SECURITY.md).
...
The front door itself authenticates no caller and speaks plain HTTP. Registration is the access gate, a tool not exposed is not routable, but any client that can reach the listen address can call every exposed tool. The `--listen` address is therefore the exposure decision: bind loopback unless the network in front is trusted, and put TLS and caller authentication in a fronting proxy if the door must leave the machine (§14).
...
| I2 | Every inter-agent tool call transits the MCP gateway, where `DENY`, `BAN`, and the build-time catalog apply. | Callers hold a gateway capability URL, never a peer address; I1 removes any alternate route (§6.1). | A call aimed anywhere but the gateway finds no route and fails inside the cage. |
...
| I9 | An operator runs exactly the bytes the author signed, at the version they signed, and a changed publisher key is caught. | Content hash re-verified on extract; ed25519 over the manifest digest plus repository plus tag; trust-on-first-use pinning (§9). | A tampered bundle, a substituted version, or a key mismatch fails the pull closed. |
...
A reference monitor that cannot be observed is only half a monitor, so every decision leaves a record. The durable per-run log captures every egress decision, allowed, denied, and pending, and is readable live or after the run with `logs`. A lifecycle and approval event feed carries run start and end, egress pending and approved, and elicitation asked and answered. Spend is recorded per run. For maximal fidelity, a run can opt in to capturing full LLM request and response payloads for later `replay`. The recording is stashed before the provider key is attached, so the artifact never holds the key, and it is written owner-readable only under the operator's home; the prompt and completion text inside it is verbatim, with no content redaction (§14). The event feed is intentionally lossy under backpressure: a subscriber that falls too far behind loses events rather than stalling the run that produces them, because a stuck observer must never block a live run.
...
- A reasoning cage's model traffic is not subject to the egress allow-set. The LLM gateway forwards to the operator's configured provider directly (§6.3), so the provider is an always-reachable destination for anything the agent can place in a prompt, including a secret it was granted.
- The catalog filter is only as strong as the catalog. A bundle that recorded none, built without introspection and declaring no `MAIN` or `EXPOSE`, leaves its edges under `DENY` and `BAN` alone, with no build-time allow-set to pin them to (§9).
- The `serve` front door trusts its network. It authenticates no caller and speaks plain HTTP; exposure is decided by the listen address, and TLS or caller authentication must come from what the operator puts in front of it (§10).
- A recorded replay stores prompts and completions verbatim. The artifact is owner-readable only and never contains the provider key, but anything an agent wrote into a prompt, including a granted secret, is in it (§13).
...
| Built-in default cage cap | 2 cpus, 512 MB memory, 1024 pids |
