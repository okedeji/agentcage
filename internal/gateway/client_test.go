...
func newTestClient(serverURL string) (*Client, *TokenMeter) {
	...
	client := NewClient(func() string { return serverURL }, "", func() time.Duration { return 5 * time.Second }, meter, budget, nil)
	return client, meter
}
...
func TestChatCompletion_Timeout(t *testing.T) {
	...
	meter := NewTokenMeter()
	budget := NewBudgetEnforcer(meter)
	client := NewClient(func() string { return srv.URL }, "", func() time.Duration { return 50 * time.Millisecond }, meter, budget, nil)
