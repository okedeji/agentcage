...
func parse(r io.Reader) (*Vesselfile, error) {
	af := &Vesselfile{
	...
	if err := scanner.Err(); err != nil {
		return nil, fmt.Errorf("reading Vesselfile: %w", err)
	}
...
func parseLine(af *Vesselfile, line string, lineNo int) error {
...
func parseFrom(af *Vesselfile, rest string, lineNo int) error {
	if af.From != "" {
...
func parseEntrypoint(af *Vesselfile, rest string, lineNo int) error {
	if af.Entrypoint != "" {
	...
	// A leading '[' is the exec form: a JSON array run without a shell, the
	// same distinction Dockerfile draws between exec and shell ENTRYPOINT. A
	// bare string stays shell form and runs via sh -c.
	if strings.HasPrefix(rest, "[") {
		var argv []string
		if err := json.Unmarshal([]byte(rest), &argv); err != nil {
			return fmt.Errorf("line %d: ENTRYPOINT exec form must be a JSON array of strings: %w", lineNo, err)
		}
		if len(argv) == 0 {
			return fmt.Errorf("line %d: ENTRYPOINT exec form is empty", lineNo)
		}
		af.EntrypointExec = argv
		af.Entrypoint = strings.Join(argv, " ")
		return nil
	}
	af.Entrypoint = rest
	return nil
...
func parseRun(af *Vesselfile, rest string, lineNo int) error {
	if rest == "" {
...
func parseModel(af *Vesselfile, rest string, lineNo int) error {
	if af.Model != nil {
...
func parseMain(af *Vesselfile, rest string, lineNo int) error {
	if af.Main != "" {
...
func parseExpose(af *Vesselfile, rest string, lineNo int) error {
	if rest == "" {
...
func parseUses(af *Vesselfile, rest string, lineNo int) error {
	if rest == "" {
...
func parseBan(af *Vesselfile, rest string, lineNo int) error {
	fields := strings.Fields(rest)
	if len(fields) == 0 {
...
func parseBudget(af *Vesselfile, rest string, lineNo int) error {
	if af.Budget != 0 {
...
func parseResources(af *Vesselfile, rest string, lineNo int) error {
	if af.Resources != nil {
...
func parseSecrets(af *Vesselfile, rest string, lineNo int) error {
	if rest == "" {
...
func parseMeta(af *Vesselfile, rest string, lineNo int) error {
...
func parseEval(af *Vesselfile, rest string, lineNo int) error {
	if af.Eval != "" {
...
func validate(af *Vesselfile) error {
	if af.From == "" {
