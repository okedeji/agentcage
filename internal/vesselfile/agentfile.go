// Package vesselfile parses and validates the Vesselfile, the declarative
...
// Vesselfile is the parsed and validated manifest for one agent. From and
...
type Vesselfile struct {
	...
	Entrypoint string // ENTRYPOINT: command line that starts the MCP server (display/shell form)
	// EntrypointExec is the exec-form ENTRYPOINT (a JSON array), run directly
	// without a shell so an agent can sit on a distroless base. nil means the
	// shell form in Entrypoint, which the image runs via sh -c.
	EntrypointExec []string
...
// Parse reads a Vesselfile from r and returns the validated result.
func Parse(r io.Reader) (*Vesselfile, error) {
	return parse(r)
}

// ParseFile is Parse for a path on disk.
func ParseFile(path string) (*Vesselfile, error) {
	f, err := os.Open(path)
	if err != nil {
		return nil, err
	}
