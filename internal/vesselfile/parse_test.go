...
func TestParse_EntrypointExecForm(t *testing.T) {
	src := `FROM ghcr.io/github/github-mcp-server:0.16.0
...
`
	...
	want := []string{"./mcpvessel", "mcp-bridge", "--", "/server/github-mcp-server", "stdio"}
	if !reflect.DeepEqual(got.EntrypointExec, want) {
		t.Errorf("EntrypointExec = %v, want %v", got.EntrypointExec, want)
	}
	// Entrypoint carries a readable join for display and validation.
	if got.Entrypoint != "./mcpvessel mcp-bridge -- /server/github-mcp-server stdio" {
...
`
	if _, err := Parse(strings.NewReader(src)); err == nil {
		t.Fatal("expected an error for malformed exec-form ENTRYPOINT")
	}
}
